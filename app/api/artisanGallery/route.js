...
import ArtisanGallery from '@/models/ArtisanGallery';

export async function POST(req) {
  await connectDB();
  try {
    const { artisanId, subImages } = await req.json();
    ...
    // Create the gallery
    const gallery = await ArtisanGallery.create({ artisan: artisanId, subImages });
    // Update the artisanGallery reference in Artisan model
    await Artisan.findByIdAndUpdate(artisanId, { artisanGallery: gallery._id }, { new: true });
...
export async function GET() {
  await connectDB();
  try {
    const galleries = await ArtisanGallery.find().populate('artisan');
...
export async function PATCH(req) {
  await connectDB();
  try {
    const { galleryId, subImages } = await req.json();
    if (!galleryId) {
    ...
    const gallery = await ArtisanGallery.findByIdAndUpdate(galleryId, update, { new: true });
    if (!gallery) {
...
export async function DELETE(req) {
  ...
  try {
    ...
    const gallery = await ArtisanGallery.findById(galleryId);
    if (!gallery) {
      return new Response(JSON.stringify({ error: 'Gallery not found' }), { status: 404 });
    }
    // Remove gallery reference from Artisan
    await Artisan.findByIdAndUpdate(gallery.artisan, { $unset: { artisanGallery: '' } });
    ...
    // Delete the gallery
    await ArtisanGallery.findByIdAndDelete(galleryId);
    if (errors.length > 0) {
