                    ...
                    group-hover:w-full data-[state=active]:w-full"></span>
                                    ...
                                    {(packageDetails.basicDetails?.fullDesc) &&
                                        <div>
                                            ...
                                            <div className="prose max-w-none">
                                                <div className="mb-4 capitalize font-semibold text-justify whitespace-pre-line" dangerouslySetInnerHTML={{ __html: packageDetails.basicDetails?.fullDesc || "" }} />
                                            </div>
                                        </div>
                                    }

                                    {/* Map */}
                                    {(packageDetails.info?.filter((item) => item.typeOfSelection === "Location Map")[0]?.selectionDesc) && <div className="pt-12">
                                ...
                                <TabsContent value="Day Plan" className="space-y-6">
                                    {packageDetails.info?.filter((info) => info.typeOfSelection === "Day Plan")?.length > 0 &&
                                        <>
                                            <h3 className="text-2xl font-bold mb-4">• Day Plan</h3>
                                            <Accordion type="single" collapsible className="w-full">
                                                {packageDetails.info?.filter((info) => info.typeOfSelection === "Day Plan")?.map((day) => (
                                                    <AccordionItem key={day._id} value={`day-${day.selectionTitle}`} className="my-4 border-black">
                                                        <AccordionTrigger className="text-left px-4 rounded-xl">
                                                            <div>
                                        ...
                                        <div className="flex md:flex-row flex-col items-start justify-evenly gap-6">
                                            ...
                                            <div className="w-full overflow-x-auto">
                                                ...
                                                {packageDetails.info
                                                    ...
                                                    ?.map((item, index) => (
                                                        <div key={index} className="prose max-w-none" dangerouslySetInnerHTML={{ __html: item.selectionDesc }} />
                                                    ))}
                                            </div>
                                            {/* Exclusions */}
                                            <div className="w-full overflow-x-auto">
                                    ...
                                    {packageDetails.info?.filter((info) => info.typeOfSelection === "Policy Content").length > 0 && <div>
                                        ...
                                        <div className="space-y-4">
                                            {packageDetails.info?.filter((info) => info.typeOfSelection === "Policy Content")?.map((policy, index) => (
                                                <div key={index}>
                                                    <h4 className="font-semibold">{policy.selectionTitle}</h4>
                                                    {policy.selectionDesc.split("\n")?.map((line, lineIndex) => (
                                                        <p key={lineIndex} className="text-gray-700 whitespace-pre-line ml-4">• {line}</p>
                                                    ))}
                                                </div>
                                            ))}
                                        </div>
