    ...
    return (
      <section className="py-14 md:py-36 xl:py-20">
        <div className="relative">
          <img
            className="absolute -top-32 left-0 -z-10 lg:scale-[2]"
            src="/bg-shape.png"
        ...
        <div className="max-w-[22rem] md:max-w-[45rem] lg:max-w-[60rem] xl:max-w-7xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
            {[...Array(4)].map((_, index) => (
              <div
                key={index}
                className="rounded-2xl group flex flex-col justify-between relative overflow-hidden w-full h-96 p-8"
              >
                {/* Skeleton for image */}
      ...
      <section className="relative py-1 w-full px-10 md:px-8 lg:px-16 overflow-hidden max-w-screen overflow-x-hidden bg-repeat-y bg-[url('/bg-custom-1.jpg')] brightness-145">
        <div className="w-full">
          ...
          <p className="text-gray-600 py-8 text-center font-barlow  w-[80%] mx-auto">
            ...
            pilgrimage tourism within India, we curate authentic and meaningful
            journeys that resonate with every spiritual seeker. From holy treks
            in the Himalayas to pilgrimages to ancient temples and sacred sites,
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
            {featuredPackages.map((item) => (
              <div
                key={item._id}
                className="rounded-2xl group flex flex-col justify-between relative overflow-hidden w-full h-96 p-0"
              >
                <Image
                  src={item.image.url}
                  width={1450}
                ...
                <div className="absolute bottom-0 left-0 text-center w-full z-20 translate-y-full group-hover:translate-y-[-30%] opacity-0 group-hover:opacity-100 transition-all duration-500 ease-in-out">
                  <h1 className="text-white text-xl xl:text-2xl mt-2 font-bold">
                    {item.title}
                  </h1>
