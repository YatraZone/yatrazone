...
const CreatePromotional = ({ artisanId, artisanDetails = null }) => {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      setReviews(
        reviews.map((r) =>
          r._id === selectedPromotion._id ? { ...r, ...updatedPromotion } : r
        )
      );
  ...
  async function fetchArtisansAndPromotions() {
    ...
    try {
      ...
      const res = await fetch(
        artisanDetails?._id || artisanId
          ? `/api/promotion?artisanId=${artisanDetails?._id || artisanId}`
          : "/api/promotion"
      );
      const data = await res.json();
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        const promoRes = await fetch(
          selectedArtisan
            ? `/api/promotion?artisanId=${selectedArtisan}`
            : "/api/promotion"
        );
  ...
  return (
    <div className="w-full max-w-4xl mx-auto">
      <h3 className="text-2xl font-bold mb-4 text-center">
        Create Promotions Testimonial / Review
      </h3>
      <form
        ref={formRef}
          ...
          <div className="border rounded p-4 text-center">
            {imagePreview && (
              <img
                ...
                className="w-32 h-32 object-cover rounded border mx-auto mb-2"
              />
            )}
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <div className="bg-white p-3 rounded border border-gray-200 shadow-md mb-2 w-1/2">
                    ...
                    <div className="text-gray-600">
                      {selectedPromotion.createdBy}
                    </div>
                  </div>
                  <div className="bg-white p-3 rounded border border-gray-200 shadow-md mb-2 w-1/2">
                    <div className="font-semibold text-gray-800">Date</div>
                    <div className="text-gray-600">
                      {formatDateDDMMYYYY(selectedPromotion.date)}
                    </div>
                  </div>
                </div>
                <div className="ProseMirror1 bg-white p-3 rounded border border-gray-200 shadow-md mb-2 max-h-28 overflow-y-auto">
                  ...
                  <div
                    dangerouslySetInnerHTML={{
                      __html: unescapeHtml(selectedPromotion.shortDescription),
                    }}
                    className="text-gray-600"
                  ></div>
                </div>
                {(selectedPromotion.image?.url ||
                  selectedPromotion.image?.key) && (
                  <div className="bg-white p-3 rounded border border-gray-200 shadow-md mb-2">
                    <div className="font-semibold text-gray-800">Image</div>
                    <img
                      src={selectedPromotion.image?.url || "No Image"}
                      alt="Promotion"
