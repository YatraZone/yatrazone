...
const ArtisanGallery = ({ artisanDetails, artisanId }) => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      const res = await fetch('/api/artisanGallery', {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
  ...
  const productTitle = artisanDetails?.firstName || "";
  ...
  useEffect(() => {
    ...
    fetch(`/api/artisanGallery?artisanId=${artisanId}`)
      .then(async res => {
        if (!res.ok) return setGalleries([]);
        const data = await res.json();
        if (Array.isArray(data)) {
          // Filter galleries by artisan ID and ensure the artisan data is populated
          setGalleries(data.filter(g => g.artisan && g.artisan._id === artisanId));
        } else {
          setGalleries([]);
        }
      })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Check if gallery exists for this artisan
      const resGallery = await fetch(`/api/artisanGallery?artisanId=${artisanId}`);
      let galleryData = null;
      if (resGallery.ok) {
        const galleriesRes = await resGallery.json();
        galleryData = Array.isArray(galleriesRes)
          ? galleriesRes.find(g => g.artisan && g.artisan._id === artisanId)
          : null;
      }
      // If gallery exists, prevent creating a duplicate
      if (galleryData && galleryData._id) {
        toast.error('A gallery for this artisan already exists. Please edit or delete the existing gallery.');
        return;
      }
      // Otherwise, create a new gallery
      const payload = { artisanId, subImages };
      const apiRes = await fetch('/api/artisanGallery', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      const apiRes = await fetch('/api/artisanGallery', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ galleryId: editGallery._id, subImages })
      });
  ...
  return (
    <div className="flex justify-center items-center py-5 w-full">
      <div className="w-full max-w-2xl">
        <h4 className="font-bold mb-4 text-center">Destination Image Gallery</h4>
        <form onSubmit={editGallery ? handleEditSubmit : handleSubmit}>
          <div className="mb-4">
            <label className="font-semibold">Destination Gallery</label>
            <Input
              type="text"
          ...
          {/* Sub Images */}
          <div className="mb-4">
            <label className="font-semibold">Destination Sub Images</label>
        ...
        <div className="mt-8">
          ...
          {loadingGalleries ? (
            ...
            <table className="w-full border text-sm">
              <thead>
                <tr>
                  <th className="border px-2 py-1 text-center">Destination Name</th>
              ...
              <tbody>
                {galleries.length === 0 ? (
                  ...
                  galleries.map(gallery => (
                    <tr key={gallery._id}>
                      <td className="border px-2 py-1 text-center">{gallery.artisan && gallery.artisan.firstName ? gallery.artisan.firstName : 'N/A'}</td>
                  ...
                  <div className="flex flex-wrap gap-2 justify-center">
                    {viewGallery.subImages && viewGallery.subImages.length > 0 ? (
                      viewGallery.subImages.map((img, i) => (
                        <img key={i} src={img.url} alt={`sub-${i}`} width={60} className="rounded border w-32" />
                      ))
                    ) : (
